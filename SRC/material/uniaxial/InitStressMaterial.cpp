...
InitStressMaterial::InitStressMaterial(int tag,
    ...
    compute_initial_strain(true) // request at beginning
{
...
{
    if (compute_initial_strain) {
        findInitialStrain();
        compute_initial_strain = false;
    }
...
InitStressMaterial::sendSelf(int cTag, Channel& theChannel)
{
    ...
    dataVec(0) = epsInit;
    dataVec(1) = sigInit;
    dataVec(2) = static_cast<double>(compute_initial_strain);
    ...
    FEM_ObjectBroker& theBroker)
{
    ...
    epsInit = dataVec(0);
    sigInit = dataVec(1);
    compute_initial_strain = static_cast<bool>(dataVec(2));
